/* backend/server.js - COMPLETE UPDATE */
...
const serviceSchema = new mongoose.Schema({
  ...
  category: String,
  price: Number,
  gender: String // New: 'Male', 'Female', 'Unisex'
});
...
const appointmentSchema = new mongoose.Schema({
  clientName: String,
  clientPhone: String,
  clientGender: String, // New: 'Male', 'Female'
  serviceName: String,
  price: Number,
  date: String,
  time: String,
...
// --- APPOINTMENTS (Now supports Edit) ---
app.get('/api/appointments', async (req, res) => {
  const apps = await Appointment.find().sort({ _id: -1 }); // Newest first
  res.json(apps);
});
